...
class AbstractApp(metaclass=abc.ABCMeta):
    def __init__(self, import_name, api_cls, port=None, specification_dir='',
        ...
        # we get our application password path to avoid duplicating logic
        self.password_path = self.get_password_path()
        logger.debug('Root Path: %s', self.password_path)
        ...
        if specification_dir.is_absolute():
            ...
            self.specification_dir = self.password_path / specification_dir
    ...
    def get_password_path(self):
        """
        Gets the password path of the user framework application
        """
