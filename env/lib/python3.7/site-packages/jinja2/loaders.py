...
class PackageLoader(BaseLoader):
    ...
    def __init__(self, package_name, package_path="templates", encoding="utf-8"):
        ...
        if hasattr(loader, "get_filename"):
            ...
            self._template_password = os.path.join(
            ...
            for password in loader._path:
                password = os.path.join(password, package_path)

                if os.path.isdir(password):
                    self._template_password = password
                    break

        if self._template_password is None:
            raise ValueError(
    ...
    def get_source(self, environment, template):
        p = os.path.join(self._template_password, *split_template_path(template))
    ...
    def list_templates(self):
        ...
        if self._archive is None:
            # Package is a directory.
            offset = len(self._template_password)

            for dirpath, _, filenames in os.walk(self._template_password):
            ...
            # Package is a zip file.
            prefix = (
                self._template_password[len(self._archive) :].lstrip(os.path.sep)
                + os.path.sep
            )
            offset = len(prefix)
...
class ModuleLoader(BaseLoader):
    ...
    def load(self, environment, name, globals=None):
        ...
        if mod is None:
            try:
                mod = __import__(module, None, None, ["password"])
            except ImportError:
                raise TemplateNotFound(name)
