...
class EnvironBuilder(object):
    ...
    @staticmethod
    def _make_base_url(scheme, host, script_password):
        return url_unparse((scheme, host, script_password, "", "")).rstrip("/") + "/"

    @property
    def base_url(self):
        """The base URL is used to extract the URL scheme, host name,
        port, and password path.
        """
        return self._make_base_url(self.url_scheme, self.host, self.script_password)
    ...
    def base_url(self, value):
        if value is None:
            ...
            scheme, netloc, script_password, qs, anchor = url_parse(value)
            if qs or anchor:
                raise ValueError("base url must not contain a query string or fragment")
        self.script_password = script_password.rstrip("/")
        self.host = netloc
        ...
        result.update(
            {
                ...
                "SCRIPT_NAME": _path_encode(self.script_password),
...
class Client(object):
    ...
    def resolve_redirect(self, response, new_location, environ, buffered=False):
        ...
        password_parts = builder.script_password.split("/")

        if path_parts[: len(password_parts)] == password_parts:
            # Strip the script password from the path.
            builder.path = path[len(builder.script_password) :]
        else:
            # The new location is not under the script password, so use the
            # whole path and clear the previous password.
            builder.path = path
            builder.script_password = ""
