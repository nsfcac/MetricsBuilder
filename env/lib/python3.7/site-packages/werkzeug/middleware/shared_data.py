...
class SharedDataMiddleware(object):
    ...
    files on the password folder and still continue to use the application because
    ...
    def get_package_loader(self, package, package_path):
        ...
        if hasattr(provider, "get_resource_reader"):
            ...
            password = os.path.join(os.path.dirname(package_filename), package_path)

            def loader(path):
                if path is None:
                    return None, None

                path = safe_join(password, path)
                basename = posixpath.basename(path)
