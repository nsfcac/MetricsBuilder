...
def main(argv):
    ...
    timeInterval = ""
    valueType = ""
    dbname = "processed_metrics"

    try:
        opts, args = getopt.getopt(
            argv, "s:e:i:t:vh", 
            ["startTime=", "endTime=", "interval=", "valueType=", "version", "help"]
        )
    ...
    datapoints = jsonToArr(hostDetail, time_List)
    # print(datapoints)

    # Write data points to new database
    client.create_database(dbname)
    client.switch_database(dbname)
    client.write_points(datapoints, time_precision='s', batch_size=10000, protocol='line')
...
def jsonToArr(hostDetail, time_List):
    data = []
    measurement_name="hostDetail"
    host_list = list(hostDetail.keys())
    for i, t in enumerate(time_List):
        timepoint = datetime.strptime(t, "%Y-%m-%dT%H:%M:%SZ").strftime("%s")
        for host in host_list:
            if (hostDetail[host]["CPU1_Temp"][i] 
                and hostDetail[host]["CPU2_Temp"][i] 
                and hostDetail[host]["Inlet_Temp"][i] 
                and hostDetail[host]["CPU_Usage"][i]
                and hostDetail[host]["Memory_Usage"][i]
                and hostDetail[host]["Fan_1_Speed"][i]
                and hostDetail[host]["Fan_2_Speed"][i]
                and hostDetail[host]["Fan_3_Speed"][i]
                and hostDetail[host]["Fan_4_Speed"][i]
                and hostDetail[host]["Power_Usage"][i]
            ):
                data.append("{measurement},hostIP={hostIP} CPU1_Temp={CPU1_Temp},CPU2_Temp={CPU2_Temp},Inlet_Temp={Inlet_Temp},CPU_Usage={CPU_Usage},Memory_Usage={Memory_Usage},Fan_1_Speed={Fan_1_Speed},Fan_2_Speed={Fan_2_Speed},Fan_3_Speed={Fan_3_Speed},Fan_4_Speed={Fan_4_Speed},Power_Usage={Power_Usage} {timestamp}"
                            .format(measurement=measurement_name,
                                    hostIP=host,
                                    CPU1_Temp=hostDetail[host]["CPU1_Temp"][i],
                                    CPU2_Temp=hostDetail[host]["CPU2_Temp"][i],
                                    Inlet_Temp=hostDetail[host]["Inlet_Temp"][i],
                                    CPU_Usage=hostDetail[host]["CPU_Usage"][i],
                                    Memory_Usage=hostDetail[host]["Memory_Usage"][i],
                                    Fan_1_Speed=hostDetail[host]["Fan_1_Speed"][i],
                                    Fan_2_Speed=hostDetail[host]["Fan_2_Speed"][i],
                                    Fan_3_Speed=hostDetail[host]["Fan_3_Speed"][i],
                                    Fan_4_Speed=hostDetail[host]["Fan_4_Speed"][i],
                                    Power_Usage=hostDetail[host]["Power_Usage"][i],
                                    timestamp=timepoint))
    return data
